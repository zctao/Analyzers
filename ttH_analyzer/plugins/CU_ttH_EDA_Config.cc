...
void CU_ttH_EDA::Load_configuration(string config_filename)
{
	const string error_message = 
		"A problem in CU_ttH_EDA::Load_configuration(..) occured while reading a config.";
	...
	if (!config["Triggers"]["HLT_muon_triggers"])
		throw std::runtime_error(error_message);
	trigger_on_HLT_muons =
		config["Triggers"]["HLT_muon_triggers"].as<string>();
	...
	if (!config["Cuts"]["min_tight_lepton_pT"])
		throw std::runtime_error(error_message);
	min_tight_lepton_pT =
		config["Cuts"]["min_tight_lepton_pT"].as<double>();
	...
	if (!config["Jets"]["jet_corrector"])
		throw std::runtime_error(error_message);
	jet_corrector =
		config["Jets"]["jet_corrector"].as<string>();
...
void CU_ttH_EDA::Load_configuration_MAODH(const string &analysis_type, bool data)
{
	if (analysis_type == "lepton+jet") {
		miniAODhelper.SetUp(MAODHelper_era,
